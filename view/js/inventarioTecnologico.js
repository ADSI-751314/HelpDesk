app.inventarioTecnologico = function() {
    ...
    $("#form-container").on('click', '.btn_inventario', function(e) {
        ...
        switch (btnId) {
            ...
            case 'btn-inv-filtrarEliminar':
                filtroParaEliminarModificarCrear();

                break;

            case 'btn-inv-consultarModificar':
                cargarModificar();
                break;
...
//            $('.validacion').html('<p style="color:red">Por favor ingrese Codigo del Historial</p>');
            ...
            $('.validacion').html("");
            ajax = objetoAjax();
            ajax.open("POST", "controller/HistorialCambios_controller.php?op=10", true);
            ajax.onreadystatechange = function() {
                if (ajax.readyState == 4) {
                    //   alert("si busco");
                    divConsulta.innerHTML = ajax.responseText;
            ...
            confirm: function() {
                ...
                ajax.onreadystatechange = function() {
                    if (ajax.readyState == 4) {
                        ...
                        MostrarConsultaEliminar();
                        showAlert("<strong>¡El Registro!</strong> " + " <strong> '" + pkCodigo + "' </strong>" + " Fue Eliminado Correctamente.");
    ...
    function  Modificar() {
        ...
        ajax.onreadystatechange = function() {
            if (ajax.readyState == 4) {
                divResultado.innerHTML = ajax.responseText;
                cargarModificar();
                showAlert(" <strong>¡Bien!</strong> Registro " + " <strong>' " + txtCodigo + "' </strong>"
